...
さらに，$N$ が十分に大きいとき，右辺に現れる補正項 $\log N$ が $N\log N$ に比べて無視できることを示せ。
...
$$
\finalanswer{\int_1^N \log x \, dx \le \log N! \le \int_1^N \log x \, dx + \log N}
$$

また

$$
\finalanswer{\frac{\log N}{N\log N}=\frac{1}{N}\longrightarrow0
\qquad (N\to\infty)}
$$

が成り立ちます。積分は $N\log N$ のオーダーであるのに対し，補正項は $\log N$ のオーダーです。その比は $1/N$ で0に近づくため，$N$ が十分に大きければ和を積分で近似できます。
...
ここで $N$ が非常に大きい極限を考えているため，定数項の $1$ は $N$ や $N \log N$ に比べて無視することができます。これにより，スターリングの公式が得られます。

$$
\finalanswer{\log N! \simeq N \log N - N}
$$
...
$$
\finalanswer{\log \Omega \simeq N \log 2}
$$
