---
tags:
  - 基本
---

# 離散確率分布の平均と分散

## 問題
ある統計的な系を考える。この系は3つの異なる微視的状態 $i=1, 2, 3$ をとり，それぞれの状態が実現する確率は以下のように与えられているものとする。

$$
P_1 = \frac{1}{2}, \quad P_2 = \frac{1}{3}, \quad P_3 = \frac{1}{6}
$$

各状態における物理量 $A$ の値が，それぞれ $A_1 = 1, A_2 = 2, A_3 = 3$ であるとき，以下の問いに答えよ。

(1) 物理量 $A$ の期待値 $\langle A \rangle$ を求めよ。

(2) 物理量 $A$ の二乗の期待値 $\langle A^2 \rangle$ を求めよ。

(3) 物理量 $A$ の分散 $\sigma_A^2 = \langle A^2 \rangle - \langle A \rangle^2$ および標準偏差 $\sigma_A$ を求めよ。

---

## 解説

### (1)

期待値の定義式 $\langle A \rangle = \sum_i P_i A_i$ に値を代入します。

$$
\begin{aligned}
\langle A \rangle
&= \frac{1}{2} \times 1 + \frac{1}{3} \times 2 + \frac{1}{6} \times 3 \\
&= \frac{1}{2} + \frac{2}{3} + \frac{1}{2} \\
&= \finalanswer{\frac{5}{3}}
\end{aligned}
$$

### (2)

二乗の期待値は $\langle A^2 \rangle = \sum_i P_i A_i^2$ として計算されます。

$$
\begin{aligned}
\langle A^2 \rangle
&= \frac{1}{2} \times 1^2 + \frac{1}{3} \times 2^2 + \frac{1}{6} \times 3^2 \\
&= \frac{1}{2} + \frac{4}{3} + \frac{9}{6} \\
&= \finalanswer{\frac{10}{3}}
\end{aligned}
$$

### (3)

分散は期待値の性質を用いて計算できます。

$$
\begin{aligned}
\sigma_A^2
&= \langle A^2 \rangle - \langle A \rangle^2 \\
&= \frac{10}{3} - \left( \frac{5}{3} \right)^2 \\
&= \frac{30}{9} - \frac{25}{9} \\
&= \finalanswer{\frac{5}{9}}
\end{aligned}
$$

標準偏差 $\sigma_A$ は分散の平方根をとることで得られます。

$$
\sigma_A = \sqrt{\frac{5}{9}} = \finalanswer{\frac{\sqrt{5}}{3}}
$$
