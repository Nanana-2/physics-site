...
$$
\begin{aligned}
\frac{d\boldsymbol{e}_r}{dt} &= \frac{d\boldsymbol{e}_r}{d\theta} \frac{d\theta}{dt} \\
...
$$
\finalanswer{\frac{d\boldsymbol{e}_r}{dt} = \dot{\theta} \boldsymbol{e}_\theta}
$$
...
$$
\begin{aligned}
\frac{d\boldsymbol{e}_\theta}{dt} &= \frac{d\boldsymbol{e}_\theta}{d\theta} \frac{d\theta}{dt} \\
...
$$
\finalanswer{\frac{d\boldsymbol{e}_\theta}{dt} = -\dot{\theta} \boldsymbol{e}_r}
$$
...
&= \frac{dr}{dt} \boldsymbol{e}_r + r \frac{d\boldsymbol{e}_r}{dt}
\end{aligned}
$$
...
$$
\finalanswer{\boldsymbol{v} = \dot{r} \boldsymbol{e}_r + r \dot{\theta} \boldsymbol{e}_\theta}
$$

各単位ベクトルの係数がそれぞれの成分に対応するため，求める成分は次のようになります。

$$
\finalanswer{v_r = \dot{r}}
$$

$$
\finalanswer{v_\theta = r \dot{\theta}}
$$
...
&= \left( \ddot{r} \boldsymbol{e}_r + \dot{r} \frac{d\boldsymbol{e}_r}{dt} \right) + \left( \dot{r} \dot{\theta} \boldsymbol{e}_\theta + r \ddot{\theta} \boldsymbol{e}_\theta + r \dot{\theta} \frac{d\boldsymbol{e}_\theta}{dt} \right)
\end{aligned}
$$
...
$$
\finalanswer{\boldsymbol{a} = (\ddot{r} - r \dot{\theta}^2) \boldsymbol{e}_r + (r \ddot{\theta} + 2 \dot{r} \dot{\theta}) \boldsymbol{e}_\theta}
$$

各単位ベクトルの係数を取り出すことで，加速度の各成分が得られます。

$$
\finalanswer{a_r = \ddot{r} - r \dot{\theta}^2}
$$

$$
\finalanswer{a_\theta = r \ddot{\theta} + 2 \dot{r} \dot{\theta}}
$$
