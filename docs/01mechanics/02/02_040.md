...
### (1)
物体には，$x$ 軸の正方向に重力 $mg$ が働き，負方向に抵抗力 $kv^2$ が働きます。
したがって，運動方程式は以下のようになります。

$$
\finalanswer{m \frac{dv}{dt} = mg - kv^2}
$$
...
$$
\finalanswer{v_{\infty} = \sqrt{\frac{mg}{k}}}
$$
...
$$
\finalanswer{v(t) = v_{\infty} \tanh \left( \frac{g}{v_{\infty}} t \right)}
$$
...
&= \finalanswer{gt - \frac{kg^2}{3m} t^3}
\end{aligned}
$$
...
$$
x(t) = \int_{0}^{t} v(t') dt' \approx \finalanswer{\frac{1}{2}gt^2 - \frac{kg^2}{12m} t^4}
$$

空気抵抗がない場合の自由落下では，速度は $v(t) = gt$，位置は $x(t) = \frac{1}{2}gt^2$ となります。
落下開始直後は自由落下の式が支配的であり，抵抗力による補正は速度では $t^3$，位置では $t^4$ から現れます。
...
$$
\finalanswer{x(t) = \frac{v_{\infty}^2}{g} \ln \cosh \left( \frac{g}{v_{\infty}} t \right)}
$$
...
&= \finalanswer{v_{\infty} t - \frac{v_{\infty}^2}{g} \ln 2}
\end{aligned}
$$
